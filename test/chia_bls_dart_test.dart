...
void main() {
  ...
  group('Test invalid inputs', () {
    ...
    for (var s in invalidInputs1) {
      test(
          'for G1 ${s.substring(0, 30)}...${s.substring(s.length - 30, s.length)}',
          () {
        var bytes_ = s.hexToBytes();
        expect(() => G1FromBytes(bytes_).checkValid(), throwsA(isA<dynamic>()));
      });
    }

    for (var s in invalidInputs2) {
      test(
          'for G2 ${s.substring(0, 30)}...${s.substring(s.length - 30, s.length)}',
          () {
        var bytes_ = s.hexToBytes();
